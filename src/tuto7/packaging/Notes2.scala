...
object Notes2  extends App {
  ...
  class Foo {
   ...
   // val map = new HashMap[String, String] :ERROR
    //on scala on peut caché un classe lors de l'importation de son package càd on importe tout les classes sauf un
    ...
    import java.util.{List => _, Map => _, Set => _, _}//on cite les classe à cacher au début puis on écrit le _ de tous les autres
