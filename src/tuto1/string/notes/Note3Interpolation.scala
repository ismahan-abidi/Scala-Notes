...
object Note3Interpolation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //pour afficher des chiffres ares la virgule on utilise la format f et %.nf
    val poids = 59
    println(f" monem pèse $poids%.0f")
    var string5 = "je \n suis la"
    println(string5)
    var string6 = raw"je \n m'appelle ismahan"
    println(string6)
    ...
    println(upper)
    // under score definit les parametre( exple:c => c) de la fonction interieur exemple lafonction map
    val upper1 = "hello, world".map(_.toUpper)
    println(upper1)
    //la methode filtre prend en parametre une fonction qui permet de filtrer les caractere selon une condtion definit dans cette fonction
